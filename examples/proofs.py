...
def run() -> None:
    ...
    Before running ensure that `cryptocurrency-advanced` service is enabled."""
    ...
    with client.protobuf_loader() as loader:
        ...
        wallet_info_response = client.get_service(
            "cryptocurrency", "v1/wallets/info?pub_key={}".format(alice_keypair.public_key.hex())
        )
...
def verify_proof_to_table(proof: Dict[Any, Any], expected_hash: Hash) -> None:
    ...
    # Keys in the proof to table are encoded as a bytes sequence parsed from hexadecimal string.
