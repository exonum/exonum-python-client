...
def run() -> None:
    """Creates a wallet in the cryptocurrency service, retrieves proofs for it and verifies them.
    Be sure to have `exonum-cryptocurrency-advanced` service instance with the name `XNM` deployed."""
    ...
    with client.protobuf_loader() as loader:
        ...
        # Map proof to the whole Exonum state hash.
        proof_to_table = wallet_info["wallet_proof"]["to_table"]
        # Expected hash for the proof to the table is state hash of the block.
        ...
        # Verify proof to table.
        verify_proof_to_table(proof_to_table, expected_to_table_hash)

        # Map proof to the wallet as a part of Cryptocurrency schema.
        proof_to_wallet = wallet_info["wallet_proof"]["to_wallet"]
        # Expected hash for the proof to the wallet is the value stored in proof to the table.
        ...
        # Verify proof to wallet.
        verify_proof_to_wallet(proof_to_wallet, expected_to_wallet_hash)

        # Map proof for the transactions associtated with the wallet.
        proof_wallet_history = wallet_info["wallet_history"]["proof"]
        # Expected hash for the wallet history is the hash stored in the proof to the wallet.
        ...
        # Verify proof for the wallet history.
...
def verify_proof_to_table(proof: Dict[Any, Any], expected_hash: Hash) -> None:
    ...
    # Keys in the proof to table are encoded as a byte sequence of (tag, group_id, index_id).
    ...
    # Values in the proof to table are encoded as a bytes sequence parsed from hexadecimal string.
...
def verify_proof_to_wallet(proof: Dict[Any, Any], expected_hash: Hash) -> None:
    ...
    # Keys in the proof to wallet are encoded as a bytes sequence parsed from hexadecimal string.
    def key_encoder(data: str) -> bytes:
        return bytes.fromhex(data)

    # Values in the proof to wallet are encoded as a protobuf binary representation of the Wallet structure.
...
def verify_wallet_history_proof(proof: Dict[Any, Any], expected_hash: Hash) -> None:
    ...
    # To convert value to bytes we can use bytes.fromhex since values are hexadecimal strings.
    try:
